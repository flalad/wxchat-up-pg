// 前端动态渐变背景
class FrontendDynamicBackground {
    constructor() {
        this.colors = [
        ...
        this.floatShapes = [];
        this.points = [];
        this.POINTS = 35; // 前端使用较少的粒子数量
        
        this.gradIndex = 0;
        this.gradNext = 1;
        this.gradTime = 0;
        this.gradDuration = 8.0; // 前端使用较慢的渐变切换
        
        this.frameCount = 0;
        this.SKIP_CONNECTION_FRAMES = 4; // 更多跳帧优化
    ...
    createCanvases() {
        ...
        this.bgCanvas.id = 'frontend-bg-canvas';
        ...
        this.effectCanvas.id = 'frontend-effect-canvas';
        this.effectCanvas.style.cssText = `
            position: absolute;
            top: 0;
            left: 0;
            width: 100vw;
    ...
    createFloatShapes() {
        this.floatShapes = [];
        for (let i = 0; i < 10; i++) { // 前端使用更少的浮动形状
            this.floatShapes.push({
                x: Math.random() * this.w,
                y: Math.random() * this.h,
                size: 50 + Math.random() * 100,
                color: this.colors[Math.floor(Math.random() * this.colors.length)],
                alpha: 0.03 + Math.random() * 0.05, // 更低的透明度
                type: Math.random() > 0.5 ? 'ellipse' : 'rect',
                dx: (Math.random() - 0.5) * 0.15, // 更慢的移动速度
                dy: (Math.random() - 0.5) * 0.15,
                rot: Math.random() * Math.PI * 2,
                drot: (Math.random() - 0.5) * 0.0008
            });
        }
    }
    ...
    createPoints() {
        ...
        for (let i = 0; i < this.POINTS; i++) {
            this.points.push({
                ...
                shape: Math.random() < 0.8 ? 'circle' : (Math.random() < 0.5 ? 'triangle' : 'star')
            });
        }
    }
    ...
    drawPoint(p) {
        ...
        this.effectCtx.globalAlpha = 0.7;
        ...
        if (p.shape === 'circle') {
            ...
            this.effectCtx.rotate(p.x / 120 + p.y / 120);
    ...
    drawLine(p1, p2) {
        this.effectCtx.save();
        this.effectCtx.globalAlpha = 0.08;
        this.effectCtx.strokeStyle = "#fff";
        this.effectCtx.lineWidth = 0.5;
        this.effectCtx.beginPath();
    ...
    drawBgGradient(dt) {
        ...
        // 绘制径向渐变
        let centerX = this.w / 2 + Math.sin(this.gradTime * 0.15) * this.w * 0.03;
        let centerY = this.h / 2 + Math.cos(this.gradTime * 0.12) * this.h * 0.03;
        
        let grad = this.bgCtx.createRadialGradient(
            stops[0].x * this.w, stops[0].y * this.h, this.w * 0.008,
            centerX, centerY, Math.max(this.w, this.h) * 1.0
        );
    ...
    animate(now) {
        ...
        for (let s of this.floatShapes) {
            ...
            if (s.type === 'ellipse') {
                this.effectCtx.beginPath();
                this.effectCtx.ellipse(0, 0, s.size * 0.6, s.size, 0, 0, 2 * Math.PI);
                this.effectCtx.fill();
            } else {
            ...
            // 边界检查
            if (s.x < -200) s.x = this.w + 150;
            if (s.x > this.w + 200) s.x = -150;
            if (s.y < -200) s.y = this.h + 150;
            if (s.y > this.h + 200) s.y = -150;
        }
...
// 自动初始化
let frontendDynamicBg = null;

document.addEventListener('DOMContentLoaded', () => {
    // 检查是否在前端页面（不是管理后台）
    if (!document.getElementById('adminApp') && !document.querySelector('.admin-header')) {
        frontendDynamicBg = new FrontendDynamicBackground();
    }
});

// 导出到全局
window.FrontendDynamicBackground = FrontendDynamicBackground;
window.frontendDynamicBg = frontendDynamicBg;